...
async def remove_challenges(
    *,
    form: ChangeChallengeForm,
    user: UserPermissionModel = Depends(get_user_permission_model),
):
    if not user.permission.Event.Write:
        raise ServiceException(
            status.HTTP_403_FORBIDDEN,
            detail="无权限",
            code=ErrorCode.REQUEST.PERMISSION_DENIED,
        )
    ...
    if len(append):
        ops.append(
            await Competition.bulk_write_append_challenge(
                form.comp_id, *[s for s in form.scores if s.id in append]
            )
        )
    if len(remove):
    ...
    await Competition.bulk_write(
        [*ops, *await Competition.bulk_write_update_score(form)]
    )
...
async def get_challenge_list(
    *, cid: str, user: UserPermissionModel = Depends(get_user_permission_model)
):
    # 是否存在比赛
    if comp := await Event.get(cid, user.id):
        ...
        if start_time > datetime.datetime.now() and not (
            user.permission.Challenge.Read or user.permission.Challenge.Write
        ):
            raise ServiceException(
                status.HTTP_403_FORBIDDEN,
                detail="无权限",
        ...
        # 是否参加/有权限
        if (
            comp.get("joined", False)
            or user.permission.Challenge.Read
            or user.permission.Challenge.Write
        ):
            if clist := await Competition.get_challenges_list(cid):
                res = []
                for cell in clist:
                    rec = await Challenge.get(cell.get("id"))
                    info = ChallengeInfoOmitFlag(
                        **rec,
                        id=str(rec["_id"]),
                        creator=str(rec["uid"]),
                        score=cell["score"],
    ...
    raise ServiceException(
        status.HTTP_404_NOT_FOUND, detail="不存在的比赛", code=ErrorCode.EVENT.NOT_FOUND
    )
...
async def check_flag(*, cfi: CheckFlagIn, uid: str = Depends(get_uid_from_token)):
    ...
    if await Competition.if_stop(cfi.comp_id):
        raise ServiceException(
            ...
            detail="已停止",
            code=ErrorCode.COMPETITION.SUBMISSION_CLOSED,
        )
...
async def get_challenge(*, cid: str, id: str, uid: str = Depends(get_uid_from_token)):
    ...
    return ChallengeInfoOmitFlag(
        **rec, id=str(rec["_id"]), creator=str(rec["uid"]), score=score
    )
